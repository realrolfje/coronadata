...
def download_intake_cumulative(force=False):
...
def download_intake_count(force=False):
...
def download_zkh_intake_count(force=False):
...
def store_intake_count(filename):
    if filename == None:
        return    
...
def store_intake_cumulative(filename):
    if filename == None:
        return    
    for record in readjson(filename):
        if dateCache.isvaliddate(record['date'], filename):
...
def store_zkh_intake_count(filename):
    if filename == None:
        return        
    for record in readjson(filename):
