...
def downloadMostRecentAppleMobilityReport(filename):
    if os.path.isfile(filename) and os.stat(filename).st_mtime > ( time.time() - 3600):
        ...
        url = 'https://covid19-static.cdn-apple.com/covid19-mobility-data/2202HotfixDev13/v3/en-us/applemobilitytrends-2021-11-16.csv'
        try:
...
def downloadECDCMap():
    ...
    with urllib.request.urlopen(url) as response:
        ...
        imglink = re.findall('https://www.ecdc.europa.eu/.*/public/images/.*Subnational_Combined_traffic.*\.png', responsebody)[0]
