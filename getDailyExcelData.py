...
with urllib.request.urlopen("https://data.rivm.nl/covid-19/COVID-19_casus_landelijk.json") as url:
...
with urllib.request.urlopen("https://www.stichting-nice.nl/covid-19/public/intake-cumulative/") as url:
    data = json.loads(url.read().decode())
...
with urllib.request.urlopen("https://www.stichting-nice.nl/covid-19/public/intake-count/") as url:
    data = json.loads(url.read().decode())
    for measurement in data:
        if (measurement['date'] not in metenisweten):
            metenisweten[measurement['date']] = {
